...
import (
	...
	"learn/models"
	"learn/restapi/operations"
	"learn/restapi/operations/todos"
)

//go:generate swagger generate server --target ../../simple-server --name TodoList --spec ../swagger.yml --principal interface{}
...
func configureAPI(api *operations.TodoListAPI) http.Handler {
	...
	api.TodosAddOneHandler = todos.AddOneHandlerFunc(func(aop todos.AddOneParams) middleware.Responder {
		if err := addItem(aop.Body); err != nil {
			return todos.NewAddOneDefault(500).WithPayload(&models.Error{
				Code:    500,
				Message: swag.String(err.Error()),
			})
		}
		return todos.NewAddOneCreated().WithPayload(aop.Body)
	})
